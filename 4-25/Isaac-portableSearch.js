    ...
    function submitChoice() {
        ...
        const matchingItem =
            primaryObj //first item that matches will be shown
                .find(search => search.name.toLowerCase() === inputElem.value.toLowerCase())
...
/*             let newHtml = `<div>Name: ${matchingItem.name}</div>`;
    ...
    inputElem.addEventListener("input", () => {
        if (inputElem.value.length > 2) {
            ...
            primaryObj
                .filter(search => search.name.toLowerCase().includes(inputElem.value.toLowerCase()))//filter according to input
                .forEach((search, i) => {//for each filtered element:
                    ...
                    newItem.innerHTML = search.name;
                    matchesElem.appendChild(newItem);

                    newItem.addEventListener("click", () => {
                        setInputValue(search.name);  // Make input match what we clicked on.
                    });
